...
(function (MixMode) {
    MixMode.AVERAGE = 1;
    MixMode.TOWARDS_PRIMARY = 2;
    MixMode.TOWARDS_SECOND = 3;
})(MixMode || (MixMode = {}));
export class AtomicServiceNavigation extends ViewPU {
    ...
    set gradientBackground(w) {
        this.__gradientBackground.set(w);
    }
    get hideTitleBar() {
        ...
        this.observeComponentCreation2((elmtId, isInitialRender) => {
            ...
            Canvas.onReady(() => {
                ...
                if (primaryColor !== undefined && secondColor === undefined) {
                    //单色渐变
                    drawSingleGradient(this.context, primaryColor);
                }
                else if (primaryColor !== undefined && secondColor !== undefined) {
                    if (mixMode === MixMode.AVERAGE) {
                        //双色渐变五五分
                        drawGradientCanvasHalf(this.context, primaryColor, secondColor);
                    }
                    else if (mixMode === MixMode.TOWARDS_PRIMARY) {
                        //第一种双色渐变三七分
                        drawGradientCanvasPrefer(this.context, primaryColor, secondColor);
                    }
                    else if (mixMode === MixMode.TOWARDS_SECOND) {
                        //第二种双色渐变三七分
                        drawGradientCanvasPrefer1(this.context, primaryColor, secondColor);
                    }
                    drawTransparentGradient(this.context);
                }
            });
        }, Canvas);
        Canvas.pop();
 ...
 * @param secondColor
 */
function drawGradientCanvasHalf(context, primaryColor, secondColor) {
    height = height * RATIO_THREE_TENTHS;
    let grad1 = context.createLinearGradient(NEGATIVE_ONE * width * HALF_RATIO, height, width * HALF_RATIO, 0);
    let grad2 = context.createLinearGradient(width * ONE_POINT_FIVE_RATIO, height, width * HALF_RATIO, 0);
 ...
 * @param secondColor
 */
function drawGradientCanvasPrefer(context, primaryColor, secondColor) {
    height = height * RATIO_THREE_TENTHS;
    ...
    let y1 = (HALF_RATIO * height - RATIO_THREE_TENTHS * width) > 0 ? HALF_RATIO * height - RATIO_THREE_TENTHS * width : 0;
    let grad2 = context.createLinearGradient(SEVENTY_PERCENT_RATIO * width, y1, width, height * HALF_RATIO);
    ...
    context.strokeRect(SEVENTY_PERCENT_RATIO * width, 0, width * RATIO_THREE_TENTHS, height * HALF_RATIO);
    context.fillRect(SEVENTY_PERCENT_RATIO * width - OFFSET_ONE, 0, width * RATIO_THREE_TENTHS + OFFSET_ONE, height * HALF_RATIO + OFFSET_ONE);
    let y2 = (HALF_RATIO * height - RATIO_THREE_TENTHS * width) > 0 ? HALF_RATIO * height + RATIO_THREE_TENTHS * width : height;
    let grad3 = context.createLinearGradient(SEVENTY_PERCENT_RATIO * width, y2, width, height * HALF_RATIO);
    ...
    context.fillRect(SEVENTY_PERCENT_RATIO * width - OFFSET_ONE, height * HALF_RATIO, RATIO_THREE_TENTHS * width + OFFSET_ONE, height * HALF_RATIO);
}
/**
 ...
 * @param secondColor
 */
function drawGradientCanvasPrefer1(context, primaryColor, secondColor) {
    height = height * RATIO_THREE_TENTHS;
 ...
 * @param context
 */
function drawTransparentGradient(context) {
 ...
 * @param primaryColor
 */
function drawSingleGradient(context, primaryColor) {
